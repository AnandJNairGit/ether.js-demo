...
const EtherDemo = () => {
  ...
  const interactSmartContractRead = async () => {
    ...
    const walletContractObj = new ethers.Contract(
      walletAddres,
      walletABI,
      provider
    );
    ...
    console.log(
      "the formatted contract balance is----------->",
      ethers.utils.formatEther(contractBalance)
    );

    const accountBalance = await walletContractObj.getAccountBalance(
      "0xDD92c9Be86316ddF0cB47a20516720E32313EF8c"
    );
    console.log("the account balance is------------->", accountBalance);
    console.log(
      "the formatted account balance is----------->",
  ...
  const interactSmartContractWrite = async () => {
    ...
    await contract.setValue(9);
    // await contract.sendETH({value:ethers.utils.parseEther("0.00005")})
    // await contract.transferETH("0x25496EC859b5AE429EaDe35ed99bA44A1D0b6641", {
    //   value: ethers.utils.parseEther("0.00005"),
    // });
  };

  const accountMethods = async () => {
    ...
    const storageSlotData = await provider.getStorageAt("0xa353b60791b301fec2700db69af541b52166cd28", 0);
    console.log("the slot data is-------->", storageSlotData);
  };
